...
def _run_worker(
    ...
    *,
    ...
    # datasets 4.x: audiofolder + data_files にフォルダを渡すと FileNotFoundError になるため
    # フォルダパスは data_dir に変換して渡す
    _data_files = _parse_data_files(args.data_files)
    _data_dir = None
    if args.dataset == "audiofolder" and _data_files is not None:
        _candidate = _data_files[0] if isinstance(_data_files, list) and len(_data_files) == 1 else (
            _data_files if isinstance(_data_files, str) else None
        )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            "DACVAE repository or local checkpoint path. "
            "Default is v2 32-dim codec."
        ),
    )
    parser.add_argument(
