...
def caption_audio(
    ...
    *,
    ...
    出力JSONL フォーマット（prepare_manifest.py との互換を意識）:
    ...
    if fmt == "csv":
        _write_csv(records, output_manifest)
    else:
...
def emoji_caption(
    ...
    *,
    ...
    """
    CSVのtextと対応するwavから音響特徴量を抽出し、LLMで絵文字キャプションを生成。
    入力CSV: file_name, text（必須カラム）
    出力CSV: file_name, text, caption（captionカラムを追加して上書き保存）
    ...
    Returns
    -------
    処理済みレコードのリスト（file_name, text, caption）
    """
    ...
    for idx, rec in enumerate(records, 1):
        ...
        for attempt in range(retry_count + 1):
            ...
            if caption is not None:
                break  # 空文字含め返答があればOK（空=元テキストのフォールバック済み）
    ...
    log_fn(f"\n絵文字キャプション完了: {len(results)}/{total} 件 → {csv_path}")
    return results
