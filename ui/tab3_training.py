...
def _start_train(
    ...
    *ui_cfg_args,
    ...
    tmp_config = cnfg.configs_dir / "_train_tmp.yaml"
    cnfg.configs_dir.mkdir(parents=True, exist_ok=True)
    ...
    log_path = cnfg.logs_dir / f"train_{stamp}.log"
...
def _write_tensorboard_events(log_path: Path) -> None:
    try:
        ...
        tb_dir = cnfg.logs_dir / "tensorboard" / log_path.stem
        ...
        if not df.empty:  # type: ignore
            csv_path = cnfg.logs_dir / f"{log_path.stem}_metrics.csv"
...
def build(ctx):
    with gr.Tab("🏋️ 学習"):
        ...
        with gr.Accordion("プリセット管理（configs/ フォルダ）", open=True):
            with gr.Row():
                preset_dropdown = gr.Dropdown(
                    label="プリセット選択",
                    choices=ctx.initial_configs,
                    value=ctx.default_config or None,
                    scale=3,
                )
        ...
        with gr.Accordion("バッチ・精度設定", open=True):
            ...
            with gr.Row():
                ...
                t_grad_accum = gr.Slider(
                    ...
                    minimum=1,
                    maximum=32,
                    value=ctx.v("gradient_accumulation_steps", 2),
                    step=1,
                )
                t_num_workers = gr.Slider(
                    ...
                    minimum=0,
                    maximum=16,
                    value=ctx.v("num_workers", 4),
                    step=1,
                )
            with gr.Row():
                t_persistent_workers = gr.Checkbox(
                    label="ワーカーの永続化（起動高速化）",
                    value=ctx.v("dataloader_persistent_workers", True),
                )
                t_prefetch_factor = gr.Slider(
                    label="プリフェッチ係数",
                    minimum=1,
                    maximum=8,
                    value=ctx.v("dataloader_prefetch_factor", 2),
                    step=1,
                )
                t_allow_tf32 = gr.Checkbox(
                    label="TF32を許可（Ampere以降GPU向け高速化）", value=ctx.v("allow_tf32", True)
                )
                t_compile_model = gr.Checkbox(
                    label="torch.compileを使用（初回遅延あり）", value=ctx.v("compile_model", False)
                )
                t_precision = gr.Dropdown(
                    label="学習精度",
                    choices=["bf16", "fp32", "fp16"],
                    value=ctx.v("precision", "bf16"),
                )
        ...
        with gr.Accordion("オプティマイザ設定", open=True):
            ...
            with gr.Row():
                t_optimizer = gr.Dropdown(
                    ...
                    value=ctx.v("optimizer", "muon"),
                )
                t_learning_rate = gr.Number(label="学習率", value=ctx.v("learning_rate", 3e-4))
                t_weight_decay = gr.Number(
                    label="重み減衰（L2正則化）", value=ctx.v("weight_decay", 0.01)
                )
            with gr.Row():
                t_muon_momentum = gr.Number(
                    label="Muonモメンタム（Muon使用時のみ有効）", value=ctx.v("muon_momentum", 0.95)
                )
                t_adam_beta1 = gr.Number(
                    label="Adam β1（AdamW使用時）", value=ctx.v("adam_beta1", 0.9)
                )
                t_adam_beta2 = gr.Number(
                    label="Adam β2（AdamW使用時）", value=ctx.v("adam_beta2", 0.999)
                )
        ...
        with gr.Accordion("学習率スケジューラ", open=True):
            ...
            with gr.Row():
                t_lr_scheduler = gr.Dropdown(
                    ...
                    value=ctx.v("lr_scheduler", "wsd"),
                )
                t_warmup_steps = gr.Number(
                    label="ウォームアップステップ数", value=ctx.v("warmup_steps", 300), precision=0
                )
                t_stable_steps = gr.Number(
                    label="安定期ステップ数（wsdのみ）",
                    value=ctx.v("stable_steps", 2100),
                    precision=0,
                )
                t_min_lr_scale = gr.Number(
                    label="最小学習率スケール比率（0〜1）", value=ctx.v("min_lr_scale", 0.01)
                )

        with gr.Accordion("学習ステップ・テキスト設定", open=True):
            with gr.Row():
                t_max_steps = gr.Number(
                    label="最大学習ステップ数", value=ctx.v("max_steps", 3000), precision=0
                )
                t_max_text_len = gr.Number(
                    label="テキスト最大トークン長", value=ctx.v("max_text_len", 256), precision=0
                )
                t_max_latent_steps = gr.Number(
                    label="ラテント最大フレーム数",
                    value=ctx.v("max_latent_steps", 750),
                    precision=0,
                )
                t_fixed_target_latent_steps = gr.Number(
                    label="固定ターゲットラテント長",
                    value=ctx.v("fixed_target_latent_steps", 750),
                    precision=0,
                )
                t_fixed_target_full_mask = gr.Checkbox(
                    label="固定ターゲット全マスク", value=ctx.v("fixed_target_full_mask", True)
                )
        ...
        with gr.Accordion("Conditioningドロップアウト・タイムステップ", open=False):
            ...
            with gr.Row():
                t_text_dropout = gr.Slider(
                    ...
                    minimum=0.0,
                    maximum=0.5,
                    value=ctx.v("text_condition_dropout", 0.15),
                    step=0.01,
                )
                t_speaker_dropout = gr.Slider(
                    ...
                    minimum=0.0,
                    maximum=0.5,
                    value=ctx.v("speaker_condition_dropout", 0.15),
                    step=0.01,
                )
                t_timestep_stratified = gr.Checkbox(
                    label="タイムステップ層化サンプリング（安定化に有効）",
                    value=ctx.v("timestep_stratified", True),
                )
        ...
        with gr.Accordion("Weights & Biases 設定", open=False):
            ...
            with gr.Row():
                t_wandb_enabled = gr.Checkbox(
                    label="W&B を有効化", value=ctx.v("wandb_enabled", False)
                )
                t_wandb_project = gr.Textbox(
                    label="W&B プロジェクト名", value=ctx.v("wandb_project", "") or ""
                )
                t_wandb_run_name = gr.Textbox(
                    label="W&B 実行名（省略可）", value=ctx.v("wandb_run_name", "") or ""
                )
        ...
        with gr.Accordion("バリデーション設定", open=False):
            ...
            with gr.Row():
                t_valid_ratio = gr.Slider(
                    ...
                    minimum=0.0,
                    maximum=0.5,
                    value=ctx.v("valid_ratio", 0.0),
                    step=0.01,
                )
                t_valid_every = gr.Number(
                    label="バリデーション実行間隔（ステップ数）",
                    value=ctx.v("valid_every", 100),
                    precision=0,
                )
        ...
        with gr.Accordion("オプション機能", open=False):
            ...
            t_seed = gr.Number(
                label="乱数シード（再現性のために固定推奨）", value=ctx.v("seed", 0), precision=0
            )
