...
def _run_convert(input_pt: str) -> str:
    ...
    cmd = f"{sys.executable} {cnfg.repo_dir / 'convert_checkpoint_to_safetensors.py'} {p}"
    try:
...
def _run_lora_convert(input_full_dir: str, force: bool = False) -> str:
    ...
    cmd = [sys.executable, str(cnfg.repo_dir / "convert_lora_checkpoint.py"), str(p)]
    if force:
        cmd += ["--force"]
    ...
    with gr.Tab("🔄 チェックポイント変換"):
        with gr.Tab("通常チェックポイント変換"):
            ...
            conv_refresh_btn.click(
                lambda: gr.Dropdown(
                    choices=scan_train_checkpoints(), value=(scan_train_checkpoints() or [None])[-1]
                ),
                outputs=[conv_input],
            )
